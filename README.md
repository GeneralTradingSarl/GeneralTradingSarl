<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00C6FF,100:0072FF&height=260&section=header&text=LADJOHOUNLOU%20Ismael&fontSize=46&fontColor=ffffff&fontAlignY=32&desc=Automation%20%C2%B7%20AI%20%C2%B7%20Energy%20%C2%B7%20Data%20%C2%B7%20Algorithmic%20Trading&descSize=18&descAlignY=48" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=0072FF&center=true&vCenter=true&width=700&lines=Building+automation+pipelines...;Training+and+deploying+AI+agents...;Engineering+data+%26+energy+systems...;Architecting+algorithmic+trading+systems..." alt="Typing SVG" />

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://ismael-portfolio-b27lpfreq-ismael-ladjohounlou-s-projects.vercel.app/en)
[![Upwork](https://img.shields.io/badge/Upwork-6FDA44?style=for-the-badge&logo=upwork&logoColor=white)](https://www.upwork.com/freelancers/~01498331f7c7800fc0)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:general.trading.bot.2025@gmail.com)

![Profile Views](https://komarev.com/ghpvc/?username=GeneralTradingSarl&label=Profile%20Views&color=0072FF&style=flat-square)
![Automation](https://img.shields.io/badge/Automation-0072FF?style=flat-square)
![AI](https://img.shields.io/badge/Artificial%20Intelligence-00C6FF?style=flat-square)
![Data](https://img.shields.io/badge/Data%20Engineering-0072FF?style=flat-square)
![Energy](https://img.shields.io/badge/Energy%20Systems-00C6FF?style=flat-square)
![Quant](https://img.shields.io/badge/Quantitative%20Finance-0072FF?style=flat-square)

</div>

## About

Multi-disciplinary engineer and freelance builder with 10+ years of experience shipping production software across **automation, artificial intelligence, data, energy, and quantitative/algorithmic trading**. I design and deploy systems end to end: from low-level MQL4/MQL5 trading robots and institutional-grade C++ pricing engines, to Python and TypeScript automation pipelines, AI agent integrations, and data platforms.

<div align="center">

<table>
<tr>
<td width="50%" valign="top">

### Automation &amp; Integration
Workflow and process automation, GUI/API automation, multi-agent orchestration, browser and desktop automation, Docker-based deployment pipelines, CI/CD.

</td>
<td width="50%" valign="top">

### AI &amp; Intelligent Agents
LLM integration, computer-use and multi-agent systems, AI-powered search and research tools, applied machine learning, natural language automation.

</td>
</tr>
<tr>
<td width="50%" valign="top">

### Data Engineering &amp; Analytics
Data pipelines and ETL automation, structured data extraction, dashboards and performance analytics, SQL/NoSQL database design, real-time monitoring.

</td>
<td width="50%" valign="top">

### Energy Systems &amp; Optimization
Energy data monitoring and forecasting, optimization models, IoT and sensor integration, applied quantitative methods for energy markets.

</td>
</tr>
</table>

</div>

### Algorithmic Trading &amp; Quantitative Finance

Expert Advisors and custom indicators for MetaTrader 4/5 (MQL4/MQL5) &middot; Smart Money Concepts (order blocks, liquidity zones, fair value gaps, market structure) &middot; quantitative pricing models (Black-Scholes, Heston, Merton, Monte Carlo, PDE) &middot; Kalman filtering &middot; GARCH volatility forecasting &middot; backtesting and risk management &middot; Pine Script strategies for TradingView.

## Tech Stack

<div align="center">
<img src="https://skillicons.dev/icons?i=py,cpp,ts,js,nextjs,nodejs,docker,git,githubactions,vercel,linux,bash" />
</div>

## Featured Projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/GeneralTradingSarl/quantsphere-terminal">quantsphere-terminal</a></h3>
      Institutional grade quant finance terminal. C++20 engine and Python. Options pricing, Kalman filtering, GARCH forecasting, Monte Carlo simulation.
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/GeneralTradingSarl/computer_Agent">computer_Agent</a></h3>
      Automation and trading experiments built on the open-source Agent S computer-use framework, with MetaTrader and knowledge-engine integrations.
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/GeneralTradingSarl/Smart-Money-Concepts">Smart-Money-Concepts</a></h3>
      Smart Money Concepts indicator for MetaTrader 5. Order blocks, liquidity zones, market structure, fair value gaps.
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/GeneralTradingSarl/Website-portfolio">Website-portfolio</a></h3>
      Static site to visualize trading account performance: account overview, trade history, and Chart.js analytics dashboards.
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/GeneralTradingSarl/mql4_experts">mql4_experts</a></h3>
      1,129+ open source MQL4 Expert Advisors for MetaTrader 4.
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/GeneralTradingSarl/expert-mt5">expert-mt5</a></h3>
      Curated, optimized MetaTrader 5 Expert Advisors with documentation.
    </td>
  </tr>
</table>

## GitHub Activity

<div align="center">
<img src="https://github-readme-streak-stats.herokuapp.com?user=GeneralTradingSarl&theme=dark&hide_border=true&background=0D1117&ring=0072FF&fire=00C6FF&currStreakLabel=0072FF" alt="GitHub Streak Stats" />
</div>

## Work With Me

Available for freelance work in automation, AI, data engineering, and algorithmic trading development. Reach out through Upwork or email above.

<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0072FF,100:00C6FF&height=120&section=footer" width="100%"/>
</div>
